...
def run_user_state_smoke_test() -> None:
    ...
    for label, req_id in archetypes:
        ...
        print(f"Current Balance:     {state.current_available_balance:,.2f} | Pending Reserve: {state.pending_reserved_amount:,.2f}")
        ...
        print(f"Upcoming (30d / 90d):30d: {state.upcoming_obligations_30d_total:,.2f} | 90d: {state.upcoming_obligations_90d_total:,.2f} (Count: {state.upcoming_obligations_count})")
